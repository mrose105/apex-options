...
OTM_PUT_MONO    = 0.050      # short put 2: 5% OTM (offset leg)
...
STOP_SPREAD_PCT = 0.85       # close if spread loses 85% of max value
...
MAX_CONCURRENT  = 3          # up to 3 overlapping positions
...
def vol_regime(closes, window=21):
    ...
    if hv < VOL_EASY:   return hv, "easy",   4.0
    if hv < VOL_NORMAL: return hv, "normal", 2.0
    if hv < VOL_HARD:   return hv, "hard",   0.75
    return                     hv, "crisis", 0.5
...
def run(data, capital, start, end):
    ...
    for today in all_dates:
        ...
        # Size: risk = max spread loss (put spread width) × contracts
        max_loss_per = (K_sp1 - K_sp2)       # = S*5% worst case
        risk_usd = portfolio * RISK_PCT * size_mult
        contracts = max(1, int(risk_usd / (max_loss_per * 100)))
        contracts = min(contracts, int(portfolio * 0.08 / (max_loss_per * 100)))
...
def report(trades, equity, capital, start, end):
    ...
    print()
    print("="*70)
    print("APEX-LADDER  —  Zero-Cost Bull Call Ladder on SPY")
    print(f"  SHORT ATM put + SHORT 5%OTM put  (full credit offset)")
...
def plot(trades, equity, stats, capital, out):
    try:
        ...
        fig.suptitle(
            "APEX-LADDER  —  Zero-Cost Bull Call Ladder  |  Short ATM+5%OTM Put  +  Long 2%/5%/8% OTM Calls",
