...
BULL_CALL_OTM  = 0.03          # buy 3% OTM call
...
# Sizing
RISK_PCT       = 0.02          # 2% portfolio per position
...
def run(data, capital, start, end):
    ...
    for today in all_dates:
        ...
        if regime=="bull":
            ...
            # Size: risk covers 3× short put premium
            contracts = max(1,int(risk/(sp_px*100*3)))
            contracts = min(contracts,int(portfolio*0.15/(sp_px*100)))
            ...
            # Size: risk = max loss = debit paid
            if debit <= 0.01: continue
            contracts = max(1,int(risk/(debit*100)))
            contracts = min(contracts,int(portfolio*0.10/(debit*100)))
            ...
            open_pos.append(pos)
            log.info(f"  {today} OPEN  BEAR-SPD "
...
def report(trades,equity,capital,start,end):
    ...
    print()
    print("="*70)
    print("APEX-HYBRID  —  Bull Risk Reversal + Bear Put Spread on SPY")
...
def plot(trades,equity,stats,capital,out):
    try:
        ...
        fig.suptitle("APEX-HYBRID  —  Bull Risk Reversal + Bear Put Spread on SPY",
